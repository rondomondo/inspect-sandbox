...
> **See [`sandbox-report.pdf`](skills/inspect-sandbox/examples/sandbox-report.pdf) for a real report generated inside the Claude sandbox.**  
...
|:---:|:---:|
| ![Report overview](skills/inspect-sandbox/examples/report.1.webp) | ![Network and mounts diagrams](skills/inspect-sandbox/examples/report.2.webp) |
...
Copies distributable files (`inspector.html`, `index.html`, `SKILL.md`, `AGENT.md`, `Makefile`, `vendor/`, `scripts/`, `favicon.ico`, `pyproject.toml`, `examples/`) into a staging directory `../zipped/inspect-sandbox/`. Used as a step in preparing a distribution zip of the skill source for installation elsewhere.
...
The skill directory (`skills/inspect-sandbox/`) can be copied to either a global or project-local skills location so the agent picks it up automatically.

**Global** (available in any project):

```bash
cp -R skills/inspect-sandbox ~/.claude/skills/
